# Lab 4: Catch all cheeses using Backtrakcing
...
def backTrakcing(remaining, vertex, weight, path, metaGraph):
    """
    ...
    if len(remaining) == 0:
        ...
        for next in remaining:
            if weight + metaGraph[vertex][next] > bestWeight:
                continue
            newRemaining.remove(next)
            newPath.append(next)
            backTrakcing(newRemaining, next, weight + metaGraph[vertex][next], newPath, metaGraph)
...
def preprocessing(mazeMap, mazeWidth, mazeHeight, playerLocation, opponentLocation, piecesOfCheese, timeAllowed):
    ...
    backTrakcing(remaining, playerLocation, 0, [(0, 0)], metaGraph)
